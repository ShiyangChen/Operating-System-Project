...
#include "Console.H"
bool FramePool::indicator[8*1024];  //initialization for static variable
...
{
	unsigned long i;	
	for(i=startframe_no;i<=startframe_no+frame_no-1;i++)
		if(indicator[i])//let's simply find first frame that is allocatable
		{
			indicator[i]=0;
			return i;
		}
