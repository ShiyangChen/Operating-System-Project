...
int main() {
   ...
   GDT::init();
    ...
    VMPool code_pool(512 MB, 256 MB, &process_mem_pool, &kernel_mem_pool, &pt1);
    VMPool heap_pool(1 GB, 256 MB, &process_mem_pool, &kernel_mem_pool, &pt1);
...
void GenerateMemoryReferences(VMPool *pool, int size1, int size2)
{
   ...
   for(int i=1; i<size1; i++) {
	  ...
	  /*if(i==11)  //i=11的时候第一次出现分配两个page的情况
	  {
		  Console::putui((unsigned int)(&(arr[1024])));
		  //for(;;);
	  }*/
      if(i==21)
	  {
		  ...
		  {
			  Console::putui((unsigned int)arr[j]);
		  }
		  for(;;);*/
		  /*for(int j=1024;j<=1030;j++)
		  {
			  Console::putui((unsigned int)arr[j]);
			  //for(;;);
		  }*/
		  //for(;;);
		  /*for(int j=2048;j<=2060;j++)
			  Console::putui((unsigned int)arr[j]);*/
		  write_cr0(read_cr0() & 0x7FFFFFFF);
		  unsigned long * pd=(unsigned long *)4194304;
		  unsigned long * pt=(unsigned long *)4202496;
		  Console::putui((unsigned int)read_cr3());
		  for(int j=0;j<=100;j++)
			  Console::putui((unsigned int)pt[j]);
	  ...
	  /*if(i==21)
	  {
		  Console::puts("Hi Buddy \n");
		  unsigned long * vaddr=(unsigned long *)0x20002000;
		  unsigned long * pd=(unsigned long *)0xFFFFF000;
		  unsigned long * b=(unsigned long *)0xFFC80000;
		  Console::putui((unsigned int)pd[128]);
		  Console::putui((unsigned int)b[0]);
		  Console::putui((unsigned int)b[1]);
		  vaddr[0]=999;
		  Console::putui((unsigned int)vaddr[0]);
		  for(;;);
	  }*/
	  if(i>=11)
	  {
		  Console::puts("Hi Buddy \n");
		  //unsigned long * vaddr=(unsigned long *)0x20001000;
		  //unsigned long * pd=(unsigned long *)0xFFFFF000;
		  //unsigned long * b=(unsigned long *)0xFFC80000;
		  write_cr0(read_cr0() & 0x7FFFFFFF);
		  //unsigned long * pd=(unsigned long *)4194304;
		  ...
		  //Console::putui((unsigned int)pd[128]);
		  //Console::putui((unsigned int)pt[0]);
		  //Console::putui((unsigned int)pt[1]);
		  write_cr0(read_cr0() | 0x80000000);
		  /*unsigned long * vaddr=(unsigned long *)0x20001000;
		  vaddr[0]=999;
		  write_cr0(read_cr0() & 0x7FFFFFFF);
